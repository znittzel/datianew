/*
 AngularJS v1.5.0-build.4493+sha.8ccc054
...
"$templateRequest","$$jqLite","$$forceReflow",function(b,c,u,l,K,q,w,oa,x,y){function M(){var a=!1;return function(b){a?b():c.$$postDigest(function(){a=!0;b()})}}function v(a,b,c){var f=F(b),d=F(a),h=[];(a=z[c])&&r(a,function(a){s.call(a.node,f)?h.push(a.callback):"leave"===c&&s.call(a.node,d)&&h.push(a.callback)});return h}function h(a,f,h){function k(c,f,h,d){u(function(){var c=v(H,a,f);c.length&&b(function(){r(c,function(b){b(a,h,d)})})});c.progress(f,h,d)}function z(b){var c=a,f=m;f.preparationClasses&&
...
var p={};p.hasTransitions=0<D.transitionDuration;p.hasAnimations=0<D.animationDuration;p.hasTransitionAll=p.hasTransitions&&"all"==D.transitionProperty;p.applyTransitionDuration=A&&(p.hasTransitions&&!p.hasTransitionAll||p.hasAnimations&&!p.hasTransitions);p.applyAnimationDuration=f.duration&&p.hasAnimations;p.applyTransitionDelay=ra(f.delay)&&(p.applyTransitionDuration||p.hasTransitions);p.applyAnimationDelay=ra(f.delay)&&p.hasAnimations;p.recalculateTimingStyles=0<S.length;if(p.applyTransitionDuration||
